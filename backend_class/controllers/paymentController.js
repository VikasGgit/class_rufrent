...
class PaymentController {
    ...
    async createOrder(req, res) {
        try {
            ...
            const { T_amount, currency } = req.body;
            // const receipt = "receipt_" + new Date().getTime();
            // Call PaymentService to create an order
            const order = await this.paymentService.createOrder({T_amount, currency});
    ...
    async verifyPayment(req, res) {
        try {
            const { razorpay_payment_id, razorpay_order_id, Inv_Id, payment_mode="Gateway" } = req.body;
